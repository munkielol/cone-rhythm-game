...
//
//  Step 2: Extract BuildNoteHeadVertices() to a shared geometry builder and
//          replace the single-segment trapezoid with N-segment curved-cap.
...
//  No NoteSkinSet, no PNG skin, no UV layout.
//
// Spec §5.7.a / §5.7.0.
...
{
    /// <summary>
    /// Production Tap note head renderer. Geometry-first (v0 step 1):
    ...
    public class TapNoteRenderer : MonoBehaviour
    {
        ...
        [Tooltip("Unlit material with _Color property support. " +
                 "Replace with texture-driven tap skin material in a later step.")]
        ...
        [Tooltip("Radial half-thickness of the tap head trapezoid in PlayfieldLocal units. " +
        ...
        [Tooltip("Base color applied via MaterialPropertyBlock._Color for active tap notes. " +
                 "Replace with texture-driven NoteSkinSet in a later step.")]
        [SerializeField] private Color tapColor = new Color(1f, 1f, 1f, 0.95f);

        [Tooltip("Color applied to Missed tap notes (dim ghost visible until past miss window).")]
        ...
        // One mesh per simultaneously visible Tap note. Vertices overwritten
        ...
        private void Awake()
        {
            ...
            {
                _meshPool[i] = BuildTrapezoidMesh("TapNoteTrapezoid");
            }
        ...
        private void LateUpdate()
        {
            ...
            if (tapMaterial == null || _meshPool == null) { return; }
            ...
            foreach (RuntimeNote note in allNotes)
            {
                ...
                Color noteColor = (note.State == NoteState.Missed) ? missedTintColor : tapColor;
                ...
                Graphics.DrawMesh(mesh, localToWorld, tapMaterial,
