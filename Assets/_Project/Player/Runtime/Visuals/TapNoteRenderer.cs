// TapNoteRenderer.cs
// Production renderer for Tap note heads.
//
// ── Architecture ─────────────────────────────────────────────────────────────
//
//  Centralized renderer: NOT attached to individual notes. Reads the global
//  note list from PlayerAppController and draws one mesh per visible Tap note
...
//
//  This step replaces the placeholder _Color-only path with the texture-driven
//  path described in spec §5.7.3:
//
//    • Material template: noteSkinSet.noteBodyMaterial
//      (shared across all note types; do NOT bake a texture into it)
//    • Body texture:      noteSkinSet.GetBodyTexture(NoteBodySkinType.Tap)
//      (assigned per draw call via MaterialPropertyBlock._MainTex)
//    • UV layout:         NoteCapGeometryBuilder.FillCapUVs(…)
//      (fixed-edge + tiled-center written into _uvScratch each frame)
...
{
    /// <summary>
    /// Production Tap note head renderer (step 3).
    /// Texture-driven skin via <see cref="NoteSkinSet"/>; CPU-driven per-frame UV
    /// assignment for fixed-edge + tiled-center body layout (spec §5.7.3).
    /// Geometry: 5-column segmented curved-cap from <see cref="NoteCapGeometryBuilder"/>.
    /// </summary>
    [AddComponentMenu("RhythmicFlow/Visuals/TapNoteRenderer")]
    public class TapNoteRenderer : MonoBehaviour
    {
        // -------------------------------------------------------------------
        // Inspector — Wiring
        // -------------------------------------------------------------------
        ...
        [Tooltip("NoteSkinSet asset driving the body material, body texture, lane width ratio, " +
                 "radial half-thickness, and missed tint for Tap notes.\n\n" +
                 "Create via Assets → Create → RhythmicFlow → Note Skin Set.\n" +
                 "Assign tapBodyTexture and noteBodyMaterial on the asset, then drag it here.")]
        ...
        [Tooltip("Optional: when assigned, frustum heights are read from this component " +
                 "so tap heads match the hold ribbon depth profile exactly. " +
                 "If null, the manual values below are used.")]
        [SerializeField] private PlayerDebugArenaSurface arenaSurface;

        [Tooltip("When true (and no arenaSurface assigned), tap heads are lifted onto " +
        ...
        // One mesh per simultaneously visible Tap note. Vertices and UVs overwritten
        ...
        // Missing tap texture (tapBodyTexture and fallbackBodyTexture both null) — warn once.
        ...
        private void LateUpdate()
        {
            ...
            if (playerAppController == null)
            {
                if (!_hasWarnedMissingController)
                {
                    ...
                    Debug.LogWarning(
                        "[TapNoteRenderer] PlayerAppController is not assigned. " +
            ...
            if (noteSkinSet == null)
            {
                if (!_hasWarnedMissingSkinSet)
                {
                    ...
                    Debug.LogWarning(
                        "[TapNoteRenderer] NoteSkinSet is not assigned. " +
            ...
            if (bodyMaterial == null || _meshPool == null)
            {
                if (bodyMaterial == null && !_hasWarnedMissingMaterial)
                {
                    ...
                    Debug.LogWarning(
                        "[TapNoteRenderer] NoteSkinSet.noteBodyMaterial is not assigned. " +
            ...
            // ── Resolve tap body texture (warn once if missing; render color-only) ─────
            // GetBodyTexture returns tapBodyTexture, falling back to fallbackBodyTexture.
            ...
            Texture2D bodyTexture = noteSkinSet.GetBodyTexture(NoteBodySkinType.Tap);
            if (bodyTexture == null && !_hasWarnedMissingTexture)
            {
                _hasWarnedMissingTexture = true;
                Debug.LogWarning(
                    "[TapNoteRenderer] NoteSkinSet has no tapBodyTexture or fallbackBodyTexture. " +
            ...
            foreach (RuntimeNote note in allNotes)
            {
                ...
                // ── Per-frame UV fill (fixed-edge + tiled-center) ─────────────────────
                ...
                //
                // _MainTex: the tap body texture. Assigned here per draw call so the shared
                ...
                mesh.uv       = _uvScratch;       // per-frame UV: fixed-edge + tiled-center
